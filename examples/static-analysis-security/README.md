...
- `threat-model.md` ou seção equivalente.
- `benchmark-card.md`, `dataset-card.md` e licença dos projetos.
- Regras/configurações versionadas e scripts de execução.
- Logs permitidos, tabela de FP/FN e análise por categoria.
- Manifesto de artefato e política de divulgação responsável.

## Conclusão responsável

O resultado deve se limitar às classes de vulnerabilidade, linguagens, projetos e condições avaliadas. Uma melhoria de recall pode ter custo em precisão ou latência; o trade-off deve aparecer na conclusão.
